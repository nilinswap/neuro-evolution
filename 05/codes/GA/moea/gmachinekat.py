...
def main():
	...
	for i in range(genlim):
		...
		for tup in newsel.select_parent(popu):
			...
			child1=newmuta.mutate(child1,prob.constraints, switch=1,iteri=i,switchiter=100,factup=(100,1000))
			child2=newmuta.mutate(child2,prob.constraints, switch=1,iteri=i,switchiter=100,factup=(100,1000))
			lis.append(child1)
			lis.append(child2)
		del(popu)
		arr=np.array(lis)
