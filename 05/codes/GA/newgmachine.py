...
def main():
	...
	while True:
		...
		for i in range(genlim):
			...
			if preverr-prevtopreverr>0.01 and nowerr-preverr>0.01:
				print("to next level")
				mrate=mrate/10
				newmuta=misc.Mutation(rate=mrate)
				prevtopreverr=2
				preverr=1
				nowerr=0
			if np.random.rand()<0.05 and abs(prevtopreverr-preverr)<0.01 and abs(nowerr-preverr)<0.01:
				print("been here***************************************************************")
			...
			if  newterm.terminate(popul=popu,iteri=i,lim=500):
				...
				flag=1
				break
			prevtopreverr=preverr
			preverr=nowerr
			nowerr=popu.avg_fitness()
		if (flag==1):
			break
