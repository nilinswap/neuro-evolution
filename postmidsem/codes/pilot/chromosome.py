...
# 0.5, 0.1, 0.2, 0.2
def aux_weighted(parent1, tup1, parent2, tup2, index):
    ...
    if fitness_tup1 <= fitness_tup2:
        ...
        theta = 0.5 * fitness_tup1[0] + 0.1 * fitness_tup1[1] + 0.2 * fitness_tup1[2] + 0.2 * fitness_tup1[3]
        fi = 0.5 * fitness_tup2[0] + 0.1 * fitness_tup2[1] + 0.2 * fitness_tup2[2] + 0.2 * fitness_tup2[3]
        if theta < fi:
            return parent1
        else:
            return parent2
...
def aux_weightedTest(parent1, tup1, parent2, tup2, index):
    fitness_tup1 = tup1
    fitness_tup2 = tup2
    if fitness_tup1 <= fitness_tup2:
        return parent1
...
def aux_non_weighted(parent1, parent2, index):  # Swapnil, if you read this, modify as you wish!
    fitness_tup1 = parent1.fitness()  # Assumed: fitness_tup contains objectives in order
    fitness_tup2 = parent2.fitness()  # Negativeloglikelihood, False +ve, False -ve, MSE
    if fitness_tup1 <= fitness_tup2:
        return parent1
    elif fitness_tup1 > fitness_tup2:
        return parent2
    else:
        if fitness_tup1[0:3] <= fitness_tup2[0:3]:
            return parent1
        elif fitness_tup1[0:3] > fitness_tup2[0:3]:
            return parent2
        elif ((fitness_tup1[0] <= fitness_tup2[0]) and (
            (fitness_tup1[1] <= fitness_tup2[1]) or (fitness_tup1[2] <= fitness_tup2[2]))):
            return parent1
        elif ((fitness_tup1[0] > fitness_tup2[0]) and (
            (fitness_tup1[1] > fitness_tup2[1]) or (fitness_tup1[2] > fitness_tup2[2]))):
            return parent2
        elif fitness_tup1[0] < fitness_tup2[0]:
            return parent1
        else:
            return parent2
...
def crossover(parent1, parent2, gen_no):
    ...
    while c1 < len1 or c2 < len2:
        ...
        if (i.innov_num == j.innov_num and f1 == f2 and f1 == 1):
            ...
            dominating_parent = aux_weighted(parent1, parent2, c1)
...
def crossoverTest(parentx, parenty, gen_no):
    parent1 = parentx[0]
    parent2 = parenty[0]
    ...
    while c1 < len1 or c2 < len2:
        ...
        if (i.innov_num == j.innov_num and f1 == f2 and f1 == 1):
            ...
            dominating_parent = aux_weightedTest(parent1, parentx[1], parent2, parenty[1], c1)
            length = len(dominating_parent.conn_arr)
            while c1 < length:
                i = dominating_parent.conn_arr[c1]
                nodeObj1 = nodeObj2 = None
...
def crossover1(parent1, parent2, gen_no, inputdim=8, outputdim=1):
    ...
    for i in range(len(parent1.bias_conn_arr)):
        alpha = random.random()  #
        new_bias_conn = gene.BiasConn(parent1.bias_conn_arr[i].out_node, (
        alpha * parent1.bias_conn_arr[i].weight + (1 - alpha) * parent2.bias_conn_arr[i].weight))
