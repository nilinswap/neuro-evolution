...
'''
	...
	def weight_mutation(self, rng, factor=0.1):
		...
		#chosen_ind = rng.choice(range(len(lis)))
		chosen_ind = random.randint(0, lis_len-1)
		modify = (rng.random() - 0.5) * 2 * factor
		if chosen_ind < len(self.conn_arr):
			self.conn_arr[chosen_ind].weight += modify
		else:
			self.bias_conn_arr[chosen_ind - len(self.conn_arr)] += modify
