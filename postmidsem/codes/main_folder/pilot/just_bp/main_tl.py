...
def test_mlp(learning_rate=0.01, L1_reg=0.00, L2_reg=0.0001, n_epochs=1000,
	...
	lis = tf_load_data.give_source_data()
	...
	with tf.Session() as sess:
		...
		lis_tar = tf_load_data.give_target_data()

		rest_set_tar = lis_tar[0]  # tuple of two shared variable of array
		test_set_tar = lis_tar[1]  # tuple of shared variable of array
		rest_setx = tf.Variable(initial_value=rest_set_tar[0], name='rest_setx', dtype=tf.float64)
		rest_sety = tf.Variable(initial_value=rest_set_tar[1], name='rest_sety', dtype=tf.int32)
		test_setx = tf.Variable(initial_value=test_set_tar[0], name='rest_sety', dtype=tf.float64)
		test_sety = tf.Variable(initial_value=test_set_tar[1], name='test_sety', dtype=tf.int32)
		...
		valid_x_to_be = rest_setx[prmsdind * par_size:(prmsdind + 1) * par_size, :]
		valid_y_to_be = rest_sety[prmsdind * par_size:(prmsdind + 1) * par_size]
		train_x_to_be = tf.concat((rest_setx[:(prmsdind) * par_size, :], rest_setx[(prmsdind + 1) * par_size:, :]),
								  axis=0)
		train_y_to_be = tf.concat((rest_sety[:(prmsdind) * par_size], rest_sety[(prmsdind + 1) * par_size:]), axis=0)
	...
	var_lis = [rest_setx, rest_sety, test_setx, test_sety]
	with tf.Session() as sess:
		sess.run([item.initializer for item in var_lis])
		# savo1.restore(sess, "/home/placements2018/forgit/neuro-evolution/05/state/tf/cards/model.ckpt")
		sess.run([classifier.logRegressionLayer.W.initializer, classifier.logRegressionLayer.b.initializer,
				  classifier.hiddenLayer.W.initializer, classifier.hiddenLayer.b.initializer])
		# print("------------------------------------------------------------------")
		# print(sess.run([valid_x_to_be,valid_y_to_be,train_x_to_be,train_y_to_be],feed_dict={prmsdind:0}))

		print(sess.run([cost], feed_dict={x: train_x_to_be.eval(feed_dict={prmsdind: zhero}),
										  y: train_y_to_be.eval(feed_dict={prmsdind: zhero})}))
		...
		for epoch in range(n_epochs):
			for ind in range(n_par):
				_, bost = sess.run([optmzr, cost], feed_dict={x: train_x_to_be.eval(feed_dict={prmsdind: ind}),
															  y: train_y_to_be.eval(feed_dict={prmsdind: ind})})
			print(bost)
			if epoch % 10 == 0:
				print(sess.run([classifier.errors(y)], feed_dict={x: valid_x_to_be.eval(feed_dict={prmsdind: ind}),
																  y: valid_y_to_be.eval(
																	  feed_dict={prmsdind: ind})}))

		st = str(sess.run(classifier.errors(y),feed_dict={x:test_setx.eval(),y:test_sety.eval()}))
		fileo = open("./log_folder/log_bp_tl_"+str(n_hidden)+"_"+str(n_epochs)+".txt", "a")
		fileo.write(st+'\n')
		fileo.close()
		print("testing",st)
